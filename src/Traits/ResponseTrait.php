...
trait ResponseTrait
{
    ...
    {
        /** Формируем структуру ответа с учетом статуса */
        $responseData = [
            'code' => $statusCode,
     ...
     * @param int $statusCode HTTP-код статуса
     * @param int|null $errorCode Код ошибки (опционально)
     * @return ResponseInterface
     */
    protected function respondWithError(
        ...
        string $message, 
        int $statusCode = 400, 
        ?int $errorCode = null): ResponseInterface
    {
        $data = [
            'message' => $message
        ];
        
        // Добавляем код ошибки, если он указан
        if ($errorCode !== null) {
            $data['code'] = $errorCode;
        }
        
        // Используем новый формат ответа с структурированными данными
        return $this->respondWithData($response, $data, $statusCode);
    }

    /**
     * Возвращает ответ с ошибкой по её коду
     *
     * @param ResponseInterface $response Объект ответа
     * @param int $errorCode Код ошибки
     * @param mixed $errorMessage Дополнительные данные об ошибке (опционально)
     * @return ResponseInterface
     */
    protected function respondWithErrorCode(ResponseInterface $response, int $errorCode, mixed $errorMessage = null): ResponseInterface
    {
