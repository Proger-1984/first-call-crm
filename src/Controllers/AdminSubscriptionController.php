...
class AdminSubscriptionController
{
    ...
    public function createSubscription(Request $request, Response $response): Response
    {
        ...
        try {
            ...
            if (!$category) {
                return $this->respondWithError($response, 'Категория не найдена', 'not_found', 404, null);
            }
            
            // Проверка существования локации
            $location = Location::find($locationId);
            if (!$location) {
                return $this->respondWithError($response, 'Локация не найдена', 'not_found', 404, null);
            }
            
            // Проверка существования тарифа
            $tariff = Tariff::find($tariffId);
            if (!$tariff) {
                return $this->respondWithError($response, 'Тариф не найден', 'not_found', 404, null);
            }
            
            // Проверка наличия активной подписки
            if ($user->hasActiveSubscription($categoryId, $locationId)) {
                return $this->respondWithError($response, 'У пользователя уже есть активная подписка для этой категории и локации', 'subscription_exists', 400, null);
            }
    ...
    public function getAllSubscriptionHistory(Request $request, Response $response): Response
    {
        ...
        try {
            $history = SubscriptionHistory::with('user')
