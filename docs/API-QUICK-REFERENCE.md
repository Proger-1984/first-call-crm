...
## CRM — Таймлайн взаимодействий (v2 Phase 2)

> Требует `Authorization: Bearer <token>` + активная подписка (`SubscriptionMiddleware`)
>
> Хронология звонков, встреч, показов, сообщений и заметок по связке объект+контакт.
>
> Типы взаимодействий:
> - `call` — звонок
> - `meeting` — встреча
...
> - `message` — сообщение
> - `note` — заметка
> - `stage_change` — смена стадии (создаётся автоматически при перемещении по воронке)
>
> Для создания вручную допускаются: `call`, `meeting`, `showing`, `message`, `note`.

### 1. Таймлайн по объекту (все контакты)
```http
GET /api/v1/properties/{id}/interactions?limit=50&offset=0
Authorization: Bearer {access_token}
```
...
|----------|-----|:---:|----------|
| `id` | integer | да | ID объекта недвижимости |

**Query-параметры:**

| Параметр | Тип | По умолчанию | Описание |
|----------|-----|:---:|----------|
| `limit` | integer | 50 | Количество записей (1–100) |
    ...
    "interactions": [
      {
        ...
        "user_id": 1,
        "type": "call",
        "description": "Обсудили условия аренды",
        "metadata": { "duration": "5 мин" },
        "interaction_at": "2026-02-13T14:30:00+03:00",
        "created_at": "2026-02-13T14:31:00+03:00",
        "contact": { "id": 5, "name": "Иванов Иван" },
        "user": { "id": 1, "name": "Сергей" }
      },
      {
        "id": 11,
        "object_client_id": 3,
        "user_id": 1,
        "type": "stage_change",
        "description": "Стадия изменена: Новый → Переговоры",
        "metadata": {
          "old_stage_id": 1,
          "new_stage_id": 2,
          "old_stage_name": "Новый",
          "new_stage_name": "Переговоры"
        },
        "interaction_at": "2026-02-12T10:00:00+03:00",
...
| Код | error | Описание |
|-----|-------|----------|
| 400 | `validation_error` | Не указан ID объекта / объект не найден |
| 401 | `unauthorized` | Не авторизован |
| 403 | `subscription_required` | Нет активной подписки |
| 500 | `internal_error` | Ошибка получения таймлайна |

---

### 2. Таймлайн по контакту (все объекты)
```http
GET /api/v1/contacts/{id}/interactions?limit=50&offset=0
Authorization: Bearer {access_token}
```
    ...
    "interactions": [
      {
        ...
        "user_id": 1,
        "type": "showing",
        "description": "Показ квартиры на Ленина 15",
        "metadata": null,
        "interaction_at": "2026-02-14T11:00:00+03:00",
        "created_at": "2026-02-13T16:00:00+03:00",
        "property": { "id": 7, "address": "ул. Ленина 15" },
        "user": { "id": 1, "name": "Сергей" }
      }
    ],
    "total": 1
...
| Код | error | Описание |
|-----|-------|----------|
| 400 | `validation_error` | Не указан ID контакта / контакт не найден |
| 401 | `unauthorized` | Не авторизован |
| 403 | `subscription_required` | Нет активной подписки |
| 500 | `internal_error` | Ошибка получения таймлайна |
...
### 3. Таймлайн конкретной связки объект+контакт
```http
GET /api/v1/properties/{id}/contacts/{contact_id}/interactions?limit=50&offset=0
Authorization: Bearer {access_token}
```
...
|----------|-----|:---:|----------|
| `id` | integer | да | ID объекта недвижимости |
| `contact_id` | integer | да | ID контакта |
    ...
    "interactions": [
      {
        ...
        "user_id": 1,
        "type": "note",
        "description": "Клиент просит скидку 5%",
        "metadata": null,
        "interaction_at": "2026-02-13T09:00:00+03:00",
        "created_at": "2026-02-13T09:00:00+03:00",
        "user": { "id": 1, "name": "Сергей" }
      }
    ],
    "total": 1
...
```
Примечание: В ответе по конкретной связке не дублируются `contact` и `property`, так как они однозначно определены path-параметрами.

**Коды ошибок:**

| Код | error | Описание |
|-----|-------|----------|
| 400 | `validation_error` | Не указаны ID объекта или контакта / объект не найден / связка не найдена |
| 401 | `unauthorized` | Не авторизован |
| 403 | `subscription_required` | Нет активной подписки |
| 500 | `internal_error` | Ошибка получения таймлайна |
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|:---:|----------|
| `type` | string | да | Тип: `call`, `meeting`, `showing`, `message`, `note` |
| `description` | string | нет | Описание / текст заметки |
| `interaction_at` | string (datetime) | нет | Дата/время (по умолчанию — текущее). Формат: `YYYY-MM-DD HH:MM:SS` |
| `metadata` | object | нет | Произвольные доп. данные (JSON) |

**Пример запроса:**
```json
{
  "type": "call",
  "description": "Обсудили условия аренды, клиент заинтересован",
  "interaction_at": "2026-02-13 14:30:00",
  "metadata": { "duration": "5 мин", "result": "перезвонить завтра" }
}
```
...
```
Примечание: При создании взаимодействий типа `call`, `meeting`, `showing` автоматически обновляется `last_contact_at` у связки объект+контакт. Тип `stage_change` создаётся автоматически при смене стадии через endpoint `/stage` и недоступен для ручного создания.

**Коды ошибок:**

| Код | error | Описание |
|-----|-------|----------|
| 400 | `validation_error` | Не указан тип / недопустимый тип / объект не найден / связка не найдена / неверный формат даты |
| 401 | `unauthorized` | Не авторизован |
| 403 | `subscription_required` | Нет активной подписки |
| 500 | `internal_error` | Ошибка создания взаимодействия |

---

## CRM — Напоминания (v2 Phase 2)

> Требует `Authorization: Bearer <token>` + активная подписка (`SubscriptionMiddleware`)
>
> Напоминания с автоматическими Telegram-уведомлениями. Cron-команда `SendRemindersCommand` отправляет уведомления в указанное время.
> Возвращаются только неотправленные (`is_sent = false`) и будущие напоминания.

### 5. Список напоминаний пользователя
```http
GET /api/v1/reminders
Authorization: Bearer {access_token}
```

Без параметров. Возвращает все будущие неотправленные напоминания текущего пользователя, отсортированные по `remind_at` (ближайшие первыми).
    ...
    "reminders": [
      {
        ...
        "object_client_id": 3,
        "user_id": 1,
        "remind_at": "2026-02-15T10:00:00.000Z",
        "message": "Перезвонить по поводу показа квартиры",
        "is_sent": false,
        "sent_at": null,
        "property": { "id": 7, "address": "ул. Ленина 15", "title": "2к квартира на Ленина" },
        "contact": { "id": 5, "name": "Иванов Иван", "phone": "+79001234567" },
        "pipeline_stage": { "id": 2, "name": "Переговоры", "color": "#FF9800" },
      ...
      },
      {
        ...
        "object_client_id": 5,
        "user_id": 1,
        "remind_at": "2026-02-16T14:00:00.000Z",
        "message": "Отправить документы на проверку",
        "is_sent": false,
        "sent_at": null,
        "property": { "id": 9, "address": "пр. Мира 42", "title": "3к квартира на Мира" },
        "contact": { "id": 8, "name": "Петров Пётр", "phone": "+79009876543" },
        "pipeline_stage": { "id": 3, "name": "Документы", "color": "#4CAF50" },
...
| 401 | `unauthorized` | Не авторизован |
| 403 | `subscription_required` | Нет активной подписки |
| 500 | `internal_error` | Ошибка получения напоминаний |

---

### 6. Напоминания по связке объект+контакт
```http
...
Возвращает неотправленные напоминания по конкретной связке объект+контакт.
...
```
Примечание: Если связка объект+контакт не найдена, возвращается пустой массив `reminders: []` (не ошибка).

**Коды ошибок:**

| Код | error | Описание |
|-----|-------|----------|
| 400 | `validation_error` | Не указаны ID объекта или контакта |
| 401 | `unauthorized` | Не авторизован |
| 403 | `subscription_required` | Нет активной подписки |
| 500 | `internal_error` | Ошибка получения напоминаний |
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|:---:|----------|
| `remind_at` | string (datetime) | да | Дата/время напоминания (должна быть в будущем). Формат: `YYYY-MM-DD HH:MM:SS` |
| `message` | string | да | Текст напоминания |

**Пример запроса:**
```json
{
  "remind_at": "2026-02-15 10:00:00",
  "message": "Перезвонить по поводу показа квартиры"
}
```
...
```json
{
  ...
  "data": {
    "reminder": {
      ...
      "contact": { "id": 5, "name": "Иванов Иван", "phone": "+79001234567" },
      "pipeline_stage": null,
      "created_at": "2026-02-13T14:31:00.000Z"
    },
    "message": "Напоминание создано"
  }
}
```
Примечание: В указанное `remind_at` время cron-команда `SendRemindersCommand` автоматически отправит Telegram-уведомление пользователю.

**Коды ошибок:**

| Код | error | Описание |
|-----|-------|----------|
| 400 | `validation_error` | Не указана дата / не указан текст / связка не найдена / дата в прошлом |
| 401 | `unauthorized` | Не авторизован |
| 403 | `subscription_required` | Нет активной подписки |
| 500 | `internal_error` | Ошибка создания напоминания |
...
```
Примечание: Удалить можно только собственные напоминания. Чужие напоминания не находятся (ответ 404).

**Коды ошибок:**

| Код | error | Описание |
|-----|-------|----------|
| 400 | `validation_error` | Не указан ID напоминания |
| 401 | `unauthorized` | Не авторизован |
| 403 | `subscription_required` | Нет активной подписки |
| 404 | `not_found` | Напоминание не найдено |
| 500 | `internal_error` | Ошибка удаления напоминания |

---

## CRM — Массовые операции (v2 Phase 2)

> Требует `Authorization: Bearer <token>` + активная подписка (`SubscriptionMiddleware`)
>
> Позволяет выполнять действия сразу над несколькими объектами. Лимит — 100 объектов за один запрос.

### 9. Массовое действие с объектами
```http
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|:---:|----------|
| `property_ids` | integer[] | да | Массив ID объектов (максимум 100) |
| `action` | string | да | Действие: `archive`, `unarchive`, `delete`, `change_deal_type` |
| `params` | object | нет | Доп. параметры для действия (см. ниже) |

**Доступные действия:**

| Действие | Описание | Параметры в `params` |
|----------|----------|----------------------|
| `archive` | Архивировать объекты | — |
| `unarchive` | Восстановить из архива | — |
| `delete` | Удалить объекты (безвозвратно) | — |
| `change_deal_type` | Изменить тип сделки | `{ "deal_type": "sale" }` или `{ "deal_type": "rent" }` |

**Пример запроса (архивирование):**
```json
{
...
**Пример запроса (смена типа сделки):**
```json
{
  "property_ids": [10, 15],
  "action": "change_deal_type",
...
```json
{
  ...
  "data": {
    "affected": 4,
    "message": "Архивировано объектов: 4"
  }
}
```
Примечание: Поле `affected` показывает реально затронутые объекты. Например, при `archive` уже архивированные объекты не учитываются. Все объекты должны принадлежать текущему пользователю.

**Коды ошибок:**

| Код | error | Описание |
|-----|-------|----------|
| 400 | `validation_error` | Не указано действие / недопустимое действие / не указаны объекты / превышен лимит 100 / объекты не найдены / недопустимый тип сделки |
| 401 | `unauthorized` | Не авторизован |
| 403 | `subscription_required` | Нет активной подписки |
| 500 | `internal_error` | Ошибка массовой операции |
